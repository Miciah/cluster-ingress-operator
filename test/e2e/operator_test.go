...
// the operator behaves correctly when the ingresscontroller's scope is mutated.
// The correct behavior depends on the platform on which the test is running.
// On AWS and IBM Cloud, the operator should set the Progressing=True status
// condition to prompt the administrator to delete the old LoadBalancer service.
// On Azure and GCP, the operator should update the LoadBalancer service's
// annotations.
