...
func (r *reconciler) ensureCRLConfigmap(ic *operatorv1.IngressController, namespace string, ownerRef metav1.OwnerReference, haveClientCA bool, clientCAConfigmap *corev1.ConfigMap) (bool, *corev1.ConfigMap, error) {
	haveCM, current, err := r.currentCRLConfigMap(ic)
	if err != nil {
	...
	case wantCM && !haveCM:
		...
		return r.currentCRLConfigMap(ic)
	case wantCM && haveCM:
		if updated, err := r.updateCRLConfigMap(current, desired); err != nil {
...
func getCRL(distributionPoints []string) (*pkix.CertificateList, error) {
	...
	for _, distributionPoint := range distributionPoints {
		...
		//
		// TODO Support ldap.
		if !strings.HasPrefix(distributionPoint, "http:") {
		...
		if err != nil {
			errs = append(errs, fmt.Errorf("error reading response from %q: %w", distributionPoint, err))
			continue
		}
		crl, err := x509.ParseCRL(bytes)
		if err != nil {
			errs = append(errs, fmt.Errorf("error parsing response from %q: %w", distributionPoint, err))
			continue
		}
		if err := kerrors.NewAggregate(errs); err != nil {
			log.Error(err, "encountered errors getting certificate distribution list")
		}
		return crl, nil
	}
...
// an error value.
func (r *reconciler) currentCRLConfigMap(ic *operatorv1.IngressController) (bool, *corev1.ConfigMap, error) {
	cm := &corev1.ConfigMap{}
	if err := r.client.Get(context.TODO(), controller.CRLConfigMapName(ic), cm); err != nil {
		if errors.IsNotFound(err) {
			return false, nil, nil
		}
		return false, nil, err
...
func (r *reconciler) updateCRLConfigMap(current, desired *corev1.ConfigMap) (bool, error) {
