...
func (r *reconciler) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	co.Status.Conditions = mergeConditions(co.Status.Conditions, computeOperatorProgressingCondition(state.IngressControllers, allIngressesAvailable, oldStatus.Versions, co.Status.Versions, r.config.OperatorReleaseVersion, r.config.IngressControllerImage, r.config.CanaryImage))
